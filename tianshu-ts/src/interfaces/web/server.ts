...
import type { TianshuApp } from "../../app.js";
...
const DB_PATH = process.env.TIANSHU_SESSION_DB || path.join(HERE, "..", "..", "..", "tianshu.db");
...
function buildApp(provider: string | null = null): TianshuApp {
...
export function createWebServer(options: { host?: string; port?: number; provider?: string | null; noToken?: boolean } = {}): Server {
  ...
  const tianshu = state.tianshu;
  tianshu.setEventHandler((agent, event, data) => {
...
async function handleWsMessage(
  ...
  if (msgType === "ask") {
    ...
    if (sid) tianshu.current_session = sid;
    else if (!tianshu.current_session && tianshu.sessions) tianshu.current_session = tianshu.newSession();
    setCurrentSession(tianshu.current_session || "");
    tianshu.clearCancel();
    send({ type: "task_start" });
    if (tianshu.isBusy()) send({ type: "queued", message: "前一个任务仍在执行,当前任务排队中..." });
const result = await runExclusive(async () => {
      const plan = await tianshu.ask(String(msg.task ?? ""), msg.use_orchestrator !== false);
    ...
    const ok = tianshu.review.decide(String(msg.review_id ?? ""), Boolean(msg.approve), "web");
...
async function route(
  ...
  try {
    ...
    if (pathName === "/api/state" && method === "GET") {
      return jsonRes(res, 200, state.tianshu.state());
    }
    ...
    if (pathName === "/api/memory" && method === "GET") {
      const out: Record<string, unknown> = { memory_stats: state.tianshu.memory_stats ?? {} };
      if (state.tianshu.cache_monitor) out.cache = state.tianshu.cache_monitor.summary();
      if (state.tianshu.memory) out.blocks = state.tianshu.memory.blockSummary();
...
const result = await runExclusive(async () => {
    ...
    if (pathName === "/api/review/decide" && method === "POST") {
      const body = await readJson(req);
      const ok = state.tianshu.review.decide(String(body.review_id ?? ""), Boolean(body.approve), "web");
...
function poolStore(state: ServerState): PoolStore {
  return state.tianshu.pool_store;
}

function rebindFromStore(t: TianshuApp): void {
