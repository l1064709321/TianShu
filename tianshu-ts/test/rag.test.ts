...
process.env.TIANSHU_RAG_DIR = path.join(fs.mkdtempSync(path.join(tmpdir(), "ts-rag-")), "rag");
...
test("BM25 检索命中相关 chunk", () => {
  ingestText("天枢系统使用 TypeScript 重写核心引擎", "a.txt", "a1");
...
test("rag_query 无 LLM 时走 fallback 回答", async () => {
  setProvider(null);
  ingestText("天枢系统使用 TypeScript 重写核心引擎,支持多 Agent 协同", "a.txt", "a2");
