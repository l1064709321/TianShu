...
before(async () => {
  ...
  process.env.TIANSHU_DEFAULT_PROVIDER = "mock-web";
  process.env.TIANSHU_PROVIDERS = JSON.stringify([
    { name: "mock-web", base_url: `http://127.0.0.1:${mockPort}/v1`, api_key: "", model: "mock-model", timeout: 30, temperature: 0.2, max_tokens: null },
  ]);
  process.env.TIANSHU_WEB_TOKEN = "test-token";
  process.env.TIANSHU_MODELS_JSON = path.join(wsTmp, "models.json");
  process.env.TIANSHU_WORKSPACE = path.join(wsTmp, "ws");
  process.env.TIANSHU_SESSION_DB = path.join(wsTmp, "sessions.db");
...
test("WebSocket:任务流与心跳", async () => {
  ...
  ws.send(JSON.stringify({ type: "ask", task: "用一句话介绍天枢", use_orchestrator: true }));
