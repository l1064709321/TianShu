...
export class Orchestrator {
  ...
  async run(task: string, planningPrompt = "", context = ""): Promise<Orchestration> {
    const stats = { total_iterations: 0, parallel_max: 0, failed_count: 0, retries: 0 };
    ...
    if (this.session_store) {
      const sid = this.main.sessionId ?? "default";
  ...
  private _waitForResult(id: string): Promise<void> {
    return new Promise<void>((resolve) => {
      const check = () => {
        ...
        if (st && (st.status === "done" || st.status === "error" || st.status === "failed")) {
          resolve();
        } else {
