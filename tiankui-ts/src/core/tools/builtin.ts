...
export async function fetchUrlGuarded(url: string, timeout = 30, _depth = 0): Promise<string> {
  ...
  try {
    resp = await fetch(url, {
      headers: { "User-Agent": "Tiankui/0.1" },
      redirect: "manual",
...
export function registerBuiltinTools(registry: ToolRegistry): void {
  ...
  registry.register(new Tool({
    name: "create_backup",
    description: "生成关键配置整包备份(config/models.json、.env、tiankui.db、身份卡片)为压缩文件 backup-*.tar.gz",
    requires_review: true,
    parameters: {
      type: "object",
  ...
  registry.register(new Tool({
    name: "restore_backup",
    description: "从备份压缩文件恢复单个关键配置(仅 models.json/.env/tiankui.db/identity-card,危险,需审批);恢复前自动建 pre-restore 备份",
    requires_review: true,
    parameters: {
      type: "object",
      properties: {
        backup: { type: "string", description: "备份文件名称" },
        target: { type: "string", description: "要恢复的目标(仅允许 models.json/.env/tiankui.db/identity-card)" },
      },
