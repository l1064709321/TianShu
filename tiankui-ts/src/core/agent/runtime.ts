...
export class Agent {
  ...
  constructor(opts: AgentOptions) {
    ...
    this.shared_context = opts.shared_context ?? {};
  ...
  async handleMessage(message: string, sender = ""): Promise<AgentResult> {
    ...
    const ctxInfo = this._formatSharedContext();
    if (ctxInfo) {
      history.push({ role: "system", content: `【共享上下文】\n${ctxInfo}` });
    }
  ...
  private _formatSharedContext(): string {
    if (!Object.keys(this.shared_context).length) return "";
    const lines = Object.entries(this.shared_context).map(([k, v]) => `- [${k}]: ${String(v).slice(0, 300)}`);
    return lines.join("\n");
  }
