...
export function createApp(
  ...
  const main = makeAgent(
    "orchestrator",
    "你是天魁主 Agent,负责接收用户任务、分解调度多个子 Agent 并汇总最终结果。" + (skillDesc ? `\n可用技能:\n${skillDesc}` : ""),
  );
...
function buildContext(app: TiankuiApp, task: string): string {
  const parts: string[] = [];
...
async function compressHistory(app: TiankuiApp): Promise<void> {
