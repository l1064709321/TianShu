#!/usr/bin/env node
const [_node, _exec, ..._rest] = process.argv;
const [majorStr] = process.versions.node.split(".");
const major = Number(majorStr);
if (major < 16) {
  console.error(`错误: 需要 Node.js >= 16.0.0，当前版本: ${process.versions.node}`);
  process.exit(1);
}
...
program
  ...
  .action(async (opts) => {
    ...
    console.log(
      "天魁已就绪。输入任务开始;/exit 退出;/agents 查看 Agent;/skills 查看技能;/pending 查看待审批;" +
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.provider) updates.push(["TIANKUI_DEFAULT_PROVIDER", opts.provider]);
    if (opts.baseUrl) updates.push(["TIANKUI_DEFAULT_PROVIDER_BASE_URL", opts.baseUrl]);
    if (opts.apiKey) updates.push(["TIANKUI_DEFAULT_PROVIDER_API_KEY", opts.apiKey]);
    if (opts.model) updates.push(["TIANKUI_DEFAULT_PROVIDER_MODEL", opts.model]);
    if (updates.length) {
    ...
    console.log(`密钥    : ${mask}`);
    console.log("修改: tiankui config --provider <name> --base-url <url> --api-key <key> --model <name>");
  });
...
program
  ...
  .action(async (opts) => {
    ...
    server.listen(Number(opts.port), opts.host, () => {
      console.log(`天魁 Web 面板: http://${opts.host}:${opts.port}/`);
    });
  });
...
program
  ...
  .action(async (opts) => {
    ...
    console.log(`天魁已就绪: http://${opts.host}:${webPort}/`);
