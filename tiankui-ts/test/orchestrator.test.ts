...
test("parse_plan 从 tool_calls 提取", () => {
  ...
  assert.ok(plan.subtasks[1].id.startsWith("st_d0_1_"));
});
...
test("编排器:空计划走直接问答", async () => {
  ...
  bus.register(main);
  const orch = new Orchestrator(main, bus, true, null, 3, 3, true, 2, async () => {});
...
test("编排器:并行子任务执行", async () => {
  ...
  const orch = new Orchestrator(main, bus, true, 2, 3, 3, true, 2, async () => {});
